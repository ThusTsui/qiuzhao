...
此信号的实现可以分为两个阶段，**信号产生**表示内核已知信号发生并修改进程的数据结构；**信号递送**表示内核执行信号处理流程。已经产生但是还没有传递的信号被称为**挂起信号**（pending signal）或者是未决信号。如果信号一直处于未决状态，那么就称进程阻塞了信号传递。
