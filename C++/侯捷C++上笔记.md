...
#### const-->常成员函数
类内函数，如果**只读取不写入**，要加const
![](images/2023-05-16-22-12-17.png)

**const成员函数只能调用带const的函数**：
> 意义是const成员函数不改变成员数据的值，所以调用的函数也不应该允许改变成员数据的值。
...
返回引用：返回本身存在的（+=）/如果是返回函数内的local变量（如+，需再定义一个新的变量存结果）这时候返回这个结果就**不能**用返回引用
